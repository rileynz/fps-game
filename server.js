...
const TDM_KILLS_TO_WIN = 20;  // team kills to win in TDM
const FFA_MAX = 20;            // max players per FFA room
const TDM_MAX = 16;            // max players per TDM room (8v8)
...
// ── Room factory ──────────────────────────────────────────────────────────────
// Each mode (ffa/tdm) has its own pool of rooms.
// A room holds its own players, bullets, round state.
const rooms = { ffa:{}, tdm:{} };
...
function randomSpawnForMap(mapId, team) {
  ...
  // TDM: use team-specific spawns if available
  const spawns = (team==='red' && map.redSpawns) ? map.redSpawns
               : (team==='blue' && map.blueSpawns) ? map.blueSpawns
               : map.spawns;
...
function assignTeam(room) {
  // Balance teams — assign to whichever has fewer players
...
function makePlayer(id, name, room) {
  ...
  const color = mode==='tdm'
    ? (team==='red' ? TEAM_RED_COLOR : TEAM_BLUE_COLOR)
    : FFA_COLORS[room.colorIdx++ % FFA_COLORS.length];
...
function roomIO(room) {
  // Helper to emit to the whole room
...
function startIntermission(room) {
  ...
  if (room.mode === 'tdm') {
    // TDM winner = team with more kills
    const winTeam = room.teamKills.red >= room.teamKills.blue ? 'red' : 'blue';
    const tied    = room.teamKills.red === room.teamKills.blue;
    room.roundWinner = {
      type: 'team',
      team: winTeam,
      color: winTeam==='red' ? TEAM_RED_COLOR : TEAM_BLUE_COLOR,
      redKills: room.teamKills.red,
      blueKills: room.teamKills.blue,
      tied,
    };
  } else {
    if (pList.length > 0) {
...
function startRound(room, mapId) {
  ...
  for (const p of Object.values(room.players)) {
    ...
    // In TDM, keep team assignment but re-spawn in team zone
    const sp = randomSpawnForMap(mapId, p.team);
    p.x=sp.x; p.y=sp.y;
  }
...
io.on('connection', socket => {
  ...
  socket.on('join', ({ name, mode: wantMode }) => {
    ...
    // Join socket.io room so we can broadcast per-room
    ...
    socket.emit('init', {
      ...
      mapName:MAPS[room.currentMapId].name,
      mapEmoji:MAPS[room.currentMapId].emoji,
      mapColor:MAPS[room.currentMapId].color,
      ...
      roundNumber:room.roundNumber,
      roster:room.roster,
      mode, myTeam:p.team, myColor:p.color,
      teamKills:room.teamKills,
      tdmKillsToWin:TDM_KILLS_TO_WIN,
      ...(room.roundState==='intermission'?{
        voteOptions:room.voteOptions.map(id=>({id,...MAPS[id],obstacles:undefined})),
  ...
  socket.on('shoot', ({angle}) => {
    ...
    myRoom.bullets.push({
      ...
      owner:socket.id, ownerTeam:p.team, ownerColor:p.color,
      life:BULLET_LIFE,
    });
  });
  ...
  socket.on('chat', ({msg, teamOnly}) => {
    ...
    if (teamOnly && myRoom.mode==='tdm' && p.team) {
      ...
      for (const [pid,tp] of Object.entries(myRoom.players)) {
        if (tp.team===p.team) io.to(pid).emit('chat', payload);
      }
    } else {
    ...
    for (const room of Object.values(rooms[mode])) {
      ...
      for (const p of pList) {
        ...
        if (dx||dy) {
          ...
          [p.x,p.y]=moveWithSlide(p.x,p.y,dx/len*PLAYER_SPEED,dy/len*PLAYER_SPEED,PLAYER_R,obs);
        }
      }
      ...
      for (const b of room.bullets) {
        ...
        for (const p of pList) {
          ...
          // TDM: skip teammates (friendly fire off)
  ...
  for (const mode of ['ffa','tdm']) {
    for (const room of Object.values(rooms[mode])) {
      ...
      const allBullets = room.roundState==='playing'
        ? room.bullets.map(b=>({id:b.id,x:b.x|0,y:b.y|0,c:b.ownerColor}))
        : [];
...
server.listen(PORT,()=>console.log(`Arena.io — FFA + TDM on port ${PORT}`));
